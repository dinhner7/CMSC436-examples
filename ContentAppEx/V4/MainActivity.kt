...
class MainActivity : AppCompatActivity() {
    ...
    inner class ListItemHandler : AdapterView.OnItemClickListener {
        override fun onItemClick(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
            // get selected Item
            var selectedItem : Item = items.get( p2 )
            // get link for the item
            var link : String = selectedItem.getLink()
            // create an Intent to start the browser
            var uri : Uri = Uri.parse( link )
            var browserIntent : Intent = Intent( Intent.ACTION_VIEW, uri )
